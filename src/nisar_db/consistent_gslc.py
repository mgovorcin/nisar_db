...
2. **Mode** — among modes with the same coverage, the standard science modes
   win in the order ``"4005"`` then ``"2005"``; non-standard (engineering /
   test) modes only compete when the frame has no standard acquisitions at all.

3. **Count** — the remaining tie-break is the number of acquisitions in the
   selected ``(mode, coverage)`` combo.

   Examples (from real NISAR data):
     - ``4005_F x5, 4005_P x2``  ->  winner ``4005_F``
     - ``4005_P x7, 4005_F x1``  ->  winner ``4005_P``  (only mode; P majority)
     - ``2005_F x4, 4005_F x4``  ->  winner ``4005_F``  (both F; 4005 beats 2005)
     - ``2005_F x4, 4005_P x4``  ->  winner ``2005_F``  (F beats P)
     - ``4005_P x7, 2005_F x1``  ->  winner ``2005_F``  (F beats P, count last)
...
def _common_mode_coverage(grp: pd.DataFrame) -> tuple[str, str]:
    ...
    ``n_F >= n_P``, else ``P``); the modes then compete on, in order:
      ...
      2. mode — :data:`~nisar_db.modes.MODE_PRIORITY` (``"4005"`` then
         ``"2005"``), with non-standard modes competing only when the frame has
         no standard acquisitions at all,
      3. count — acquisitions in the selected ``(mode, coverage)`` combo.
    ...
    ranked = pd.DataFrame(
        {
            ...
            "coverage": candidates["F"].ge(candidates["P"]).map({True: "F", False: "P"}),
...
def main(
    ...
    Selection logic:
      ...
      2. Winning mode: full-frame beats partial, then "4005" beats "2005",
         then acquisition count
