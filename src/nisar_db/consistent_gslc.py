...
# Fraction of a frame's acquisitions that must be partial before partial
# coverage outranks full. Preferring ``F`` costs temporal coverage whenever the
# frame is mostly observed partially -- a frame seen as ``4005_P`` four times
# and ``2005_F`` once yields a one-epoch stack. Above this share the partial
# series is the longer one and wins instead. The value is a judgement call, not
# a derived quantity.
...
def _common_mode_coverage(grp: pd.DataFrame) -> tuple[str, str]:
    ...
    ``n_F >= n_P``, else ``P``); the modes then compete on, in order:
      ...
      1. coverage — full-frame (``F``) beats partial (``P``), unless partial
         ...
         the frame, in which case the preference flips,
